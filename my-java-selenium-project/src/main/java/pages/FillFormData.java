...
public class MITPageCheckoutFlow {
    // ... WebElement fields and constructor ...

    // Existing fillFormData method (as you already have)
    public void fillFormData(Map<String, String> data) {
        logger.info("Filling Checkout form data from map: {}", data.keySet());

        if (data.containsKey("syfPartnerId")) setSyfPartnerId(data.get("syfPartnerId"));
        if (data.containsKey("transAmount")) setTransAmount(data.get("transAmount"));
        if (data.containsKey("promoOverrideFlag")) selectPromoOverrideFlag(data.get("promoOverrideFlag"));
        ...
        if (data.containsKey("promotionalTags")) setPromotionalTags(data.get("promotionalTags"));
        if (data.containsKey("productInfo")) setProductInfo(data.get("productInfo"));
    ...
    // New method to load YAML and fill form for a given node (e.g., "valid_user")
    public void fillFormDataFromYaml(String nodeName) {
        Map<String, String> data = YamlDataUtil.getKeyMapFromNode("data.yaml", nodeName);
        fillFormData(data);
    }
}